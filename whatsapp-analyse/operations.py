...
# Special Messages that indicate if media was there in the message, message was deleated
#
...
#
SEARCH_TERM_GRP_DESC_CHANGE: str = "changed the group description" # messg example (date, time removed): K changed the group description
SEARCH_TERM_GRP_VIDEO_CALL: str = "started a video call" # messg example (date, time removed): K started a video call
SEARCH_TERM_GRP_ICON_CHANGE: str = "changed this group's icon" # messg example (date, time removed): K changed this group's icon

SEARCH_TERM_GRP_CREATOR: str = "created group" # messg example (date, time removed): K created group "EXAMPLE"
SPLIT_TERM_GRP_CREATOR_NAME: str = "created"
SPLIT_TERM_GRP_CREATOR_GRP_NAME: str = "group"

SEARCH_TERM_GRP_NAME_CHANGE: str = "changed the subject from" # messg example (date, time removed): K changed the subject from  "EXAMPLE" to "example"
        ...
        for identifier in STRINGS_USED_TO_IDENTIFY_LINKS:
            ...
            for word in messg.split():
                if identifier in word:
                    lst_of_links.append(word.strip()) # no break here as a single message may habe > 1 link
                link_found = True
...
def sum_of_words(categorised_data_key: list) -> int:
    ...
    for date_time_messg in categorised_data_key:
        messg: str = date_time_messg[2]
        total_words += len(re.findall(RE_FOR_ALL_WORDS, messg))   # This regular expression finds all the words in the string
    return total_words
...
def sum_of_char(categorised_data_key: list) -> int:
    ...
    for date_time_messg in categorised_data_key:
        messg: str = date_time_messg[2]
        for word in re.findall(RE_FOR_ALL_WORDS, messg):  # This regular expression finds all the words in the string
...
def list_of_words(categorised_data_key: list) -> list:
    ...
    for date_time_messg in categorised_data_key:
        messg:str = date_time_messg[2]
        word_list += re.findall(RE_FOR_ALL_WORDS, messg)     # This regular expression finds all the words in the string
    return word_list
...
def clean_word_list(word_lst: list) -> dict:
    ...
    for word in word_lst:
        if any(not chars_in_word.isalpha() for chars_in_word in word): # Assumption, words made of alphabets (of any script)
            continue
...
def notif_data(catagorised_data_key: list) -> dict:
    """
    ...
    so for that portion, only the way the name of the group changed over time and by whome is shown
    But for the rest, the data of what the change was doesn't exist so more focus has been but who
    made the changes
    """

    # We have both a individual counter and total counter, as in this case we want both, and didn't want
    # the individual parts to be computed to produce the total later
    data:dict = {}
    data["Group Creator"] = ""
    data["Group Discription Change"] = [0, {}]  # total count, record of changes
    data["Group Video Call"] = [0, {}]  # total count, record of changes
    data["Group Icon Change"] = [0, {}]  # total count, record of changes
    data["Group Name"] = []  # Just record, in form of list of tuple

    for date_time_messg in catagorised_data_key:
        date = date_time_messg[0] # time is determined to be not a useful matric here
        messg = date_time_messg[2]

        if SEARCH_TERM_GRP_DESC_CHANGE in messg:
            person_name: str = messg.split(SEARCH_TERM_GRP_DESC_CHANGE)[0]

            temp_get_data: list = data["Group Discription Change"][1].get(person_name, [0, []])
            temp_get_data[0] += 1
            temp_get_data[1] += [date]
            data["Group Discription Change"][1][person_name] = temp_get_data

            data["Group Discription Change"][0] += 1
